...
**Fix (already applied):** WiFiManager is isolated in `src/WifiPortal.cpp`, so it can include `WebServer.h` without colliding with the `HTTP_*` enum names that ESPAsyncWebServer defines.  Keep `WiFiManager.h` out of any file that includes `ESPAsyncWebServer.h`.
